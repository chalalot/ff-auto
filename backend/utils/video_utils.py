...
class ProgressLogger(ProgressBarLogger):
    """Generic progress logger (replaces Streamlit-specific one)."""
    ...
    def bars_callback(self, bar, attr, value, old_value=None):
        if self.progress_handler and bar in self.bars and "total" in self.bars[bar]:
            ...
            if total > 0:
                progress = max(0.0, min(1.0, value / total))
    ...
    try:
        ...
        if transition_type == "Crossfade" and len(clips) > 1:
            ...
            for i, clip in enumerate(clips):
                ...
                processed_clips.append(clip.with_effects(effects) if effects else clip)
        ...
        logger = ProgressLogger(progress_callback) if progress_callback else "bar"
